...
class DashboardSnapshotApiTests(unittest.TestCase):
    def test_tail_helpers_return_recent_rows_without_full_file_semantics(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            jsonl_path = root / "events.jsonl"
            text_path = root / "collector.log"
            append_jsonl(jsonl_path, [{"idx": idx, "secret": "must-not-leak"} for idx in range(100)])
            ...
            json_tail = read_jsonl_tail(jsonl_path, 3)
            text_tail = read_text_tail(text_path, 2)

        self.assertEqual([row["idx"] for row in json_tail], [97, 98, 99])
        self.assertEqual(json_tail[-1]["secret"], "[REDACTED]")
        self.assertEqual(text_tail[0], "Authorization: Bearer [REDACTED]")
        self.assertEqual(text_tail[1], "tail line")

...
class MixedResearchInputTests(unittest.TestCase):
    def test_mixed_loader_counts_shadow_and_testnet_without_collapsing_raw_branches(self) -> None:
        with TemporaryDirectory() as temp_dir:
            ...
            write_jsonl(
                ...
                [make_testnet_trade("t-1", net_pnl_usdt=2.0), make_testnet_trade("t-2", net_pnl_usdt=-3.0)],
            )
    ...
    def test_testnet_independent_event_dedup_uses_stable_trade_identity(self) -> None:
        with TemporaryDirectory() as temp_dir:
            ...
            duplicate = make_testnet_trade("same-trade", net_pnl_usdt=1.5)
            write_jsonl(
                trades_file,
                [
                    duplicate,
                    {**duplicate, "net_pnl_usdt": -2.5},
                    make_testnet_trade("other-trade", net_pnl_usdt=0.8),
                ],
            )
    ...
    def test_cli_accepts_testnet_dir_and_reports_cross_validation(self) -> None:
        with TemporaryDirectory() as temp_dir:
            ...
            write_jsonl(testnet / "round_001_trades.jsonl", [make_testnet_trade("t-1", strategy="breakout", net_pnl_usdt=-2.0)])

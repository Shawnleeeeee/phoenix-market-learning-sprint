...
def evaluate_promotion_gate(
    ...
    *,
    ...
    legacy_testnet_candidate_ok = (
        str(experiment_result.get("mode") or "").lower() == "testnet_candidate"
        and checks["sample_enough"]
        and checks["net_pnl_positive"]
        and first_metric(candidate, "avg_return_pct", "avg_return") is not None
        and safe_float(first_metric(candidate, "avg_return_pct", "avg_return"), 0.0) > 0
        and checks["profit_factor_ok"]
        and checks["monte_carlo_ok"]
        and checks["drawdown_ok"]
        and checks["fee_drag_not_worse"]
        and checks["take_profit_net_loss_not_worse"]
    )
    if legacy_testnet_candidate_ok:
        legacy_optional_blockers = {
            "missing_after_cost_edge",
            "missing_expected_net_edge_bps",
            "missing_expected_gross_move_bps",
            "missing_expected_total_cost_bps",
            "evidence_level_too_low_for_testnet",
            "missing_entry_parity_pass",
            "missing_symbol_concentration",
        }
        blockers = [item for item in blockers if item not in legacy_optional_blockers]

    status = "promoted_testnet_candidate" if legacy_testnet_candidate_ok and not blockers else ("manual_review_required" if not blockers else "blocked")

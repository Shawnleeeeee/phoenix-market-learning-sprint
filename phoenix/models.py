...
class SymbolRules:
    ...
    def from_dict(cls, payload: dict[str, Any]) -> "SymbolRules":
        return cls(
            ...
            price_precision=(
                int(payload["price_precision"])
                if payload.get("price_precision") not in (None, "")
                else None
            ),
            quantity_precision=(
                int(payload["quantity_precision"])
                if payload.get("quantity_precision") not in (None, "")
                else None
            ),
            market_lot_min_qty=(
                float(payload["market_lot_min_qty"])
                if payload.get("market_lot_min_qty") not in (None, "")
                else None
            ),
            market_lot_step_size=(
                float(payload["market_lot_step_size"])
                if payload.get("market_lot_step_size") not in (None, "")
                else None
            ),
        )

...
LifecycleMonitorFactory = Callable[[dict[str, Any]], Awaitable[dict[str, Any]] | dict[str, Any]]
...
class TestnetExecutorCallback:
    ...
    def __init__(
        ...
        *,
        ...
        lifecycle_monitor_factory: LifecycleMonitorFactory | None = None,
        ...
        self.lifecycle_monitor_factory = lifecycle_monitor_factory
        self.lifecycle_monitor_enabled = bool(lifecycle_monitor_enabled or lifecycle_monitor_factory is not None)
        self.lifecycle_poll_sec = max(0.0, float(lifecycle_poll_sec))
        self.lifecycle_post_close_wait_sec = max(0.0, float(lifecycle_post_close_wait_sec))
    ...
    async def __call__(self, execution_intent: dict[str, Any]) -> dict[str, Any]:
        ...
        micro_reasons = _validate_micro_quote_allocation(self.quote_allocation_usdt)
        if micro_reasons:
            return _frozen_executor_result(",".join(micro_reasons), executor_stage="micro_notional_preflight")
        ...
        async with aiohttp.ClientSession(timeout=timeout) as session:
            ...
            explicit_stop_price = _optional_float(raw_decision.get("stop_loss_price"))
            explicit_take_profit_price = _optional_float(raw_decision.get("take_profit_price"))
            if explicit_stop_price is not None:
                intent = replace(intent, initial_stop_price=explicit_stop_price)
            if explicit_take_profit_price is not None:
                intent = replace(intent, take_profit_price=explicit_take_profit_price)
            ...
            take_profit = _find_instruction(plan, "initial_take_profit")
            if entry is None:
            ...
            if take_profit is None:
                return _frozen_executor_result("take_profit_path_unavailable", executor_stage="order_plan_preflight")
            ...
            try:
                ...
                take_profit_response = await submit_binance_order_intent(
                    futures,
                    take_profit.payload,
                    snapshot=protection_snapshot,
                    environment=_testnet_gateway_environment(environment.name),
                    source="hermes_trader_mode:testnet_take_profit",
                    purpose="take_profit",
                    endpoint=take_profit.endpoint,
                    order_intent=intent,
                    dry_run=False,
                    audit_log_path=risk_log_path,
                    extra_context={
                ...
                failed_stage = "take_profit_failed" if "take_profit_response" not in locals() and "stop_response" in locals() else "protective_stop_failed"
                freeze_reason = failed_stage if emergency.get("ok") else "emergency_close_failed"
                return {
                    ...
                    "status": f"{failed_stage}_emergency_close_attempted",
                    "entry_response": entry_response,
                    "protective_stop_response": locals().get("stop_response"),
            ...
            result = {
                ...
                "status": "testnet_order_submitted_with_protective_stop_and_take_profit",
                "entry_response": entry_response,
            ...
            if self.lifecycle_monitor_enabled:
                lifecycle_result = await self._run_lifecycle_monitor(
                    futures=futures,
                    execution_intent=execution_intent,
                    executor_payload=result,
                    environment_name=environment.name,
                    risk_log_path=risk_log_path,
                )
                result["lifecycle_result"] = lifecycle_result
                if lifecycle_result.get("hard_freeze"):
                    result.update(
                        {
                            "frozen": True,
                            "can_continue": False,
                            "result_type": "hard_freeze",
                            "freeze_reason": lifecycle_result.get("freeze_reason")
                            or lifecycle_result.get("final_close_reason")
                            or "lifecycle_monitor_hard_freeze",
                            "status": "lifecycle_monitor_hard_freeze",
                        }
                    )
                elif lifecycle_result.get("final_close_reason"):
                    result["status"] = "testnet_order_submitted_with_protective_stop_take_profit_and_lifecycle_closed"
            return result

    async def _run_lifecycle_monitor(
        self,
        *,
        ...
        from phoenix.stage2_lifecycle_monitor import Stage2LifecycleMonitor, Stage2ProtectedPosition

        protected_position = Stage2ProtectedPosition.from_executor_result(
            execution_intent=execution_intent,
        ...
        context = {
            ...
            "risk_log_path": risk_log_path,
            "poll_sec": self.lifecycle_poll_sec,
            "post_close_wait_sec": self.lifecycle_post_close_wait_sec,
        }
        if self.lifecycle_monitor_factory is not None:
            result = self.lifecycle_monitor_factory(context)
        ...
        return await monitor.monitor_until_closed()
...
def validate_trusted_runtime_snapshot(snapshot: dict[str, Any]) -> list[str]:
    ...
    if status.get("take_profit_path_available") is not True:
        reasons.append("take_profit_path_unavailable")
    if not _verified_capability(status.get("take_profit_capability_source")):
        reasons.append("take_profit_path_unverified")
...
def verify_testnet_executor_capabilities() -> dict[str, Any]:
    ...
    return {
        ...
        "take_profit_path_available": bool(take_profit_ok),
        "take_profit_capability_source": (
            "verified_code_path:PhoenixExecutor.build_order_plan+BinanceFuturesClient.new_conditional_order"
            if take_profit_ok
            else "unverified"
        ),
        "take_profit_order_supported": bool(take_profit_ok),
...
def _validate_micro_quote_allocation(value: Any) -> list[str]:
    try:
        quote = float(value)
    except (TypeError, ValueError):
        return ["micro_notional_required"]
    if quote <= 0:
        return ["micro_notional_required"]
    if quote > 10.0:
        return ["micro_notional_exceeded"]
    return []

...
DEFAULT_MAX_EXACT_LINE_COUNT_BYTES = 128 * 1024 * 1024
...
def read_jsonl_tail(path: Path, limit: int = DEFAULT_MAX_JSONL_ROWS) -> list[dict[str, Any]]:
    ...
    try:
        with path.open("rb") as handle:
            handle.seek(0, 2)
            size = handle.tell()
            read_size = min(size, max(256 * 1024, limit * 64 * 1024))
            handle.seek(size - read_size)
            raw_lines = handle.read(read_size).splitlines()
    except OSError:
        return []
    rows: deque[dict[str, Any]] = deque(maxlen=max(0, limit))
    for raw in raw_lines[-max(1, limit * 4) :]:
...
def read_text_tail(path: Path, limit: int = 40) -> list[str]:
    ...
    try:
        with path.open("rb") as handle:
            handle.seek(0, 2)
            size = handle.tell()
            read_size = min(size, max(128 * 1024, limit * 16 * 1024))
            handle.seek(size - read_size)
            raw_lines = handle.read(read_size).splitlines()
    except OSError:
        return []
    lines: deque[str] = deque(maxlen=max(0, limit))
    for raw in raw_lines[-max(1, limit * 2) :]:
...
def count_lines(path: Path) -> int | None:
    return count_lines_from(path, offset=0)


def count_lines_from(path: Path, *, offset: int = 0) -> int | None:
    try:
        total = 0
        with path.open("rb") as handle:
            if offset > 0:
...
class DashboardSnapshotService:
    def __init__(
        ...
        *,
        ...
        max_exact_line_count_bytes: int = DEFAULT_MAX_EXACT_LINE_COUNT_BYTES,
        ...
        self.max_exact_line_count_bytes = max(0, int(max_exact_line_count_bytes))
        self.testnet_env_file = testnet_env_file
        self._signed_get = signed_get
        self._cache_until = 0.0
        self._cache: dict[str, Any] | None = None
        self._line_count_cache: dict[str, dict[str, Any]] = {}
        self._line_count_lock = threading.Lock()
    ...
    def _build_snapshot(self) -> dict[str, Any]:
        ...
        shadow_signal_count = self._count_lines_cached(signals_path)
        shadow_outcome_count = self._count_lines_cached(outcomes_path)
        pending_orders = [
    ...
    def _count_lines_cached(self, path: Path) -> int | None:
        key = str(path)
        try:
            stat = path.stat()
        except OSError:
            with self._line_count_lock:
                self._line_count_cache[key] = {
                    "status": "missing",
                    "count": None,
                    "sizeBytes": None,
                    "mtimeNs": None,
                    "maxExactBytes": self.max_exact_line_count_bytes,
                }
            return None
        ...
        with self._line_count_lock:
            ...
            if (
                cached
                and cached.get("status") in {"exact", "incremental"}
                and cached.get("sizeBytes") == stat.st_size
            ...
            if (
                cached
                and cached.get("status") in {"exact", "incremental", "cache_hit"}
                and isinstance(cached.get("count"), int)
                and isinstance(cached.get("sizeBytes"), int)
                and stat.st_size >= int(cached["sizeBytes"])
            ):
                delta = count_lines_from(path, offset=int(cached["sizeBytes"]))
                if delta is None:
                    self._line_count_cache[key] = {
                        "status": "read_error",
                        "count": None,
                    ...
                    return None
                count = int(cached["count"]) + int(delta)
                self._line_count_cache[key] = {
                    "status": "incremental",
                    "count": count,
                    "sizeBytes": stat.st_size,
            ...
            if stat.st_size > self.max_exact_line_count_bytes:
                self._line_count_cache[key] = {
                    "status": "skipped_large_uncached",
                    "count": None,
                    "sizeBytes": stat.st_size,
                    "mtimeNs": stat.st_mtime_ns,
                    "maxExactBytes": self.max_exact_line_count_bytes,
            ...
            self._line_count_cache[key] = {
                "status": "exact" if count is not None else "read_error",
                "count": count,
                "sizeBytes": stat.st_size,
                "mtimeNs": stat.st_mtime_ns,
                "maxExactBytes": self.max_exact_line_count_bytes,
    ...
    def _line_count_diagnostic(self, path: Path) -> dict[str, Any]:
        with self._line_count_lock:
            cached = dict(self._line_count_cache.get(str(path), {}))
        return {
            "path": str(path),
            "status": cached.get("status", "not_requested"),
            "sizeBytes": cached.get("sizeBytes"),
            "count": cached.get("count"),
            "maxExactBytes": cached.get("maxExactBytes", self.max_exact_line_count_bytes),
        }
    ...
    def _sample_counts(
        ...
        *,
        ...
        shadow_all = shadow_all if isinstance(shadow_all, dict) else {}
        return {
            "shadowSignals": shadow_signal_count if shadow_signal_count is not None else self._count_lines_cached(signals_path),
            "shadowOutcomes": shadow_outcome_count if shadow_outcome_count is not None else self._count_lines_cached(outcomes_path),
            ...
            "eventFeed": self._count_lines_cached(collector_root / "bridge_event_feed.jsonl"),
            "eventSnapshots": self._count_lines_cached(collector_root / "event_snapshots.jsonl"),
            "eventHorizonLabels": self._count_lines_cached(collector_root / "event_horizon_labels.jsonl"),
            "lineCountDiagnostics": sanitize(
                {
                    "shadowSignals": self._line_count_diagnostic(signals_path),
                    "shadowOutcomes": self._line_count_diagnostic(outcomes_path),
                    "eventFeed": self._line_count_diagnostic(collector_root / "bridge_event_feed.jsonl"),
                    "eventSnapshots": self._line_count_diagnostic(collector_root / "event_snapshots.jsonl"),
                    "eventHorizonLabels": self._line_count_diagnostic(collector_root / "event_horizon_labels.jsonl"),
                }
            ),
        }
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument("--max-exact-line-count-bytes", type=int, default=DEFAULT_MAX_EXACT_LINE_COUNT_BYTES)
...
def main(argv: list[str] | None = None) -> int:
    ...
    service = DashboardSnapshotService(
        ...
        max_exact_line_count_bytes=args.max_exact_line_count_bytes,
